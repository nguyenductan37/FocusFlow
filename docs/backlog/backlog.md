...
Cao
8
Tích hợp SDK @google/genai phía Client và cấu hình VITE_GEMINI_API_KEY.
✅ Đã refine (Sử dụng Gemini 2.5 Flash thông qua Structured Outputs với JSON Schema để ép kiểu đầu ra tiếng Việt chính xác.)
MVP critical. Giảm friction tâm lý khi nhập task. Đã giải quyết spike kỹ thuật bằng API SDK phía client.
PB-F2
Feature
...
Cao
8
Cơ chế đếm ngược thời gian thực, giao diện Overlay toàn màn hình (Modal/CSS)
✅ Đã refine (Thống nhất phạm vi khóa: chỉ khóa trong phạm vi giao diện Web App bằng full-screen overlay modal chặn click chuột & phím ESC, kèm hộp thoại xác nhận beforeunload.)
Tăng tỷ lệ hoàn thành Deep Work. Tránh xung đột kỹ thuật sandbox trình duyệt bằng in-app lock.
PB-F3
Feature
...
- PB-F1 (AI Auto-Triage): Đã quyết định sử dụng SDK @google/genai gọi API trực tiếp ở Client-side thông qua biến VITE_GEMINI_API_KEY.
- PB-F6 (Focus Mode): Đã thống nhất phạm vi khóa chỉ trong tab trình duyệt Web App (In-app Focus Lock) bằng Fullscreen Overlay và chặn tương tác chuột/bàn phím.
